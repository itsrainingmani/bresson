...
const F_NUMBERS: [u32] = [1, 2, 3, 4, 5, 8, 11, 16, 22, 32, 45, 64];
...
impl RandomMetadata {
    pub fn random_datetime(&mut self) -> String {
        let now_utc = Utc::now();
    ...
    pub fn randomize(&mut self, tag_to_modify: Tag) -> Option<Value> {
        ...
        if self.tags_to_randomize.contains(&tag_to_modify) {
            match tag_to_modify {
                ...
                Tag::DateTimeOriginal | Tag::DateTime | Tag::DateTimeDigitized => {
                    Some(Value::Ascii(vec![Vec::from(self.random_datetime())]))
